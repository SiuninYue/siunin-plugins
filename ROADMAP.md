...
| 49 | `feature-breakdown` | **功能拆解师** | ✅ 已有 | 将大功能拆分为可独立交付的小任务。粒度：单任务 15 分钟~2 小时。拆解前重读产品合同，确保每个子任务可独立验收 |
| 50 | `feature-implement` | **功能实现者（标准）** | ✅ 已有 | 按拆解好的任务实现功能。标准复杂度。STEP 0：重读产品合同+设计规范；设计稿为实现最高优先级参考 |
| 51 | `feature-implement-simple` | **功能实现者（简单）** | ✅ 已有 | 简单功能的快速实现。同样遵循 STEP 0 重读文档 + 设计优先规则 |
| 52 | `feature-implement-complex` | **功能实现者（复杂）** | ✅ 已有 | 复杂功能的分阶段实现。每个 face 开始前强制重读产品合同+设计规范，防止长任务需求漂移 |
...
### 交付：Feedback Observer（内置反馈捕获）

**放在哪里**：PROG 内置 sub-agent，在每次对话中静默运行。

**Detect Feedback Signal 机制**：
- 自动检测用户消息中的修正/不满意信号——
  "不对""重做""不是这样""换一种""错了""不要"等关键词触发记录
- **静默记录**，不打断用户，后台写入 `.prog/feedback/{project}.jsonl`
- 半自动——检测到信号后提醒主 agent 确认是否记录，避免误报

**与 feedback-triage 的分工**：
| 管道 | 来源 | 处理方式 |
|------|------|---------|
| feedback-triage | 外部用户（GitHub Issues、社群、邮件） | 主动分类、关联产品合同 |
| Feedback Observer | 内部对话（你和 AI 的互动修正） | 静默捕获、信号累积 |

两条管道最终汇聚到 M3 Learnings 系统，共同驱动进化。
...
Learnings 不能只是被动积累，必须有**主动进化机制**——把反复出现的信号升级为系统规则。

```
触发条件                                → 动作
────────────────────────────────────────────────────────────
同一反馈/修正信号被记录 ≥3 次             → 生成"升级为当前 skill 正式规则"建议
某个 skill 的 review 评分连续 3 次偏低    → 生成"优化该 skill 方法论"建议
出现 ≥3 次未被任何 skill 覆盖的重复操作    → 生成"创建新 skill"建议
```

**执行方式**：
- Evolution Runner 在每次 session 启动时扫描 `.prog/feedback/` 和 learnings 记录
- 达到阈值 → 生成人类可读的进化提案，**不自动应用**
- 人类确认后，变更记录写入 Evolution Log，可追溯"这条规则何时、因何被加入"
- 进化系统后台运行，仅在满足阈值时浮现到前台

**与 Sub Agent 隔离的关系**：进化产生的规则 → 经人类确认 → 成为 Learnings → 在新 task 开始时注入。
隔离保证 task 执行不受污染，Learnings 保证经验不丢失。两者互补，不矛盾。
...
10. 每次开发前重读产品合同和设计规范，防止 AI 在长任务中产生隐性需求漂移
11. 设计稿是实现的最高优先级参考——设计稿 > 产品合同 > 开发者自行判断
12. 关键操作有 hook 兜底——编译不过不能 commit、未 review 不能标记 done、检测到修正信号自动触发反馈记录

### v1.0 执行视图：8 阶段管线

完整的公司闭环可抽象为 8 个独立可触发的阶段，作为 ROADMAP 工作流的简化入口：

```
01 Product Spec  →  02 Design Brief  →  03 Design Maker
04 Dev Planner   →  05 Dev Builder   →  06 Bug Fixer
07 Code Review   →  08 Release Builder
```

- 每个阶段**独立触发/独立执行**，不强依赖前后阶段
- 每个阶段背后由 SPM 或 PROG 的具体 skill 提供执行能力（见第一节清单）
- 阶段之间允许回环——Bug Fixer 可能触发需求重新澄清，Code Review 可能触达 Design Brief
- **Dry-Run 模式**：任何阶段可切换为"仅验证、不写代码"模式——
  让 AI 走一遍流程，在纸面上暴露逻辑漏洞，零成本试错。"先编排再开发"
...
[ ] Feedback Observer + Detect Signal  ← Milestone 2
[ ] 进化系统三级阈值                    ← Milestone 3
[ ] autoplan                         ← Milestone 6
[ ] 自治运营循环                       ← Milestone 6
[ ] 跨插件路由标准化                    ← Milestone 6
[ ] Stop Gate hook（未 review 阻止 done）← Milestone 6
[ ] Dry-Run 验证模式                   ← Milestone 6
```
...
> **三条操作铁律**（每次执行时生效）：
>
> 1. **先编排再开发**：在写任何代码前，先用 skill 跑通流程（dry-run），在纸面上验证逻辑一致性。
>    避免方向性返工。用编排成本替代开发成本。
> 2. **设计稿是实现阶段的宪法**：开发时的一切判断以设计稿为准。
>    优先级链：设计稿 > 产品合同 > 开发者自行判断。
> 3. **每个 face 前重读文档**：feature-implement 的每个实现阶段开始前，
>    强制重新阅读产品合同和设计规范。AI 在长任务中会逐渐"忘记"原始需求，这是唯一的防漂移机制。
...
SPM:office-hours ── "那我们来写第一版产品合同"──→ docs/product-contracts/
  │ 产出: Goals / Scope / AC / Risks（面向 AI 执行者设计，人类审核确认）
  │
  ▼
...
7. **Sub Agent 隔离。**
   每个 task/sub-agent 使用全新实例，不复用/继承上下文。
   错误假设在 task 边界被截断，确保干净入、干净出。
   与 Learnings 注入不矛盾——Learnings 是经确认的显式知识，
   隔离防的是执行过程中产生的未验证假设。这是 AI agent 工程最
   关键的防御性设计。

8. **AI 优先文档。**
   产品合同、设计规范、开发计划的第一受众是 AI 执行者。
   文档 = AI 可解析的接口定义：每条 AC 无歧义、无隐式前提、可被独立验证。
   人类可读是硬约束（用于审核和决策），但不能以牺牲 AI 可执行性为代价。
