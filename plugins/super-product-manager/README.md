...
| `/meeting` | 记录会议纪要、行动项并同步到 progress-tracker |
| `/roundtable` | 记录多角色圆桌讨论并同步决策更新 |
| `/assign` | 为 feature 分配 architecture/coding/testing 负责人并同步 |
| `/followup` | 更新行动项状态并同步 status/handoff 更新 |
| `/office-hours` | 生成产品合同（目标/范围/验收/风险），不含技术实现路径 |
| `/plan-ceo-review` | 对产品合同执行机会与风险评审并同步 planning 更新 |
| `/plan-design-review` | 执行设计质量评审（可选 lane，自动建议） |
| `/plan-devex-review` | 执行 DevEx 摩擦评审（可选 lane，自动建议） |
