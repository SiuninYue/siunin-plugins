...
- **轻量检查点** - 自动快照写入 `docs/progress-tracker/state/checkpoints.json`（不创建 git 提交）
...
# → 系统架构设计
# → 生成 docs/progress-tracker/architecture/architecture.md
...
存储在项目的 `docs/progress-tracker/state/` 目录中：
...
- 架构主文档：`docs/progress-tracker/architecture/architecture.md`
- 功能执行计划：`docs/progress-tracker/plans/feature-*.md`
- `workflow_state.plan_path` 必须始终指向 `docs/progress-tracker/plans/*.md`
...
当 `docs/progress-tracker/architecture/architecture.md` 存在时，`/prog init` 会：
...
**计划**: docs/progress-tracker/plans/2024-01-24-registration-api.md
...
**SessionStart 钩子**检查：
1. `docs/progress-tracker/state/progress.json` 是否存在？
2. 是否有未完成的功能？
...
**UserPromptSubmit 钩子**：
- 每 30 分钟自动创建轻量检查点
- 快照（含 phase/task/branch/worktree）保存到 `docs/progress-tracker/state/checkpoints.json`
- 不执行 git commit
...
- ✅ 新增 `architectural-planning` 技能：
  ...
  - 系统架构设计指导
  - 架构决策记录（ADR）生成
  - 保存到 `docs/progress-tracker/architecture/architecture.md`
