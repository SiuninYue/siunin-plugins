...
from lock_manager import (
    ...
    _PROGRESS_LOCK_HANDLES,
    _PROGRESS_LOCK_DEPTHS,
    _progress_lock_path,
...
from git_utils import (
    _run_git,
    _get_dirty_state_files,
    _git_commit_state,
    _auto_state_commit,
    _detect_default_branch,
    _git_squash_close_task,
    analyze_git_sync_risks,
    git_sync_check,
    _resolve_upstream,
    _remove_worktree,
...
from worktree_handler import (
    ...
    _extract_branch_name_from_worktree_ref,
    _count_branch_commits_behind,
    _find_existing_worktree_candidates_for_feature,
    _local_and_origin_ref_candidates,
...
from route_preflight import (
    _scope_hint,
    _extract_command_tail,
    _discover_parent_route_bindings_for_child,
    _print_route_preflight_block,
    enforce_route_preflight,
)
from evaluator_gateway import (
    _emit,
    reconcile_evaluator,
    _collect_ship_signals,
)
...
# LINKED_SNAPSHOT_SCHEMA_VERSION, TRACKER_ROLES, DEFAULT_TRACKER_ROLE imported from state_io
...
# _iter_linked_project_specs, _resolve_linked_project_root, _get_main_repo_root
# moved to route_sync.py (F18 modularisation). Re-imported below so existing call
# sites (and test patches) continue to resolve via the progress_manager namespace.
...
# _count_feature_completion, _is_linked_snapshot_stale, _detect_parallel_active_routes,
# _normalize_project_code, _serialize_project_root_for_config,
# _load_progress_payload_at_root, _format_route_feature_ref, _upsert_active_route,
# _remove_active_route, _notify_parent_sync moved to route_sync.py (F18 modularisation).
# Re-imported above so existing call sites (and test patches) continue to resolve
# via the progress_manager namespace.
# collect_linked_project_statuses and sync_linked remain here (not extracted per F18 plan).
...
# _derive_plugin_code, _generate_project_code, _discover_plugin_catalog,
# _link_child_to_parent, _auto_discover_child_plugins, discover_children,
# route_status, prioritize_route, set_routing_queue, _resolve_repo_root,
# route_select moved to route_commands.py (F18 modularisation).
# Re-imported below so existing call sites (and test patches) continue to
# resolve via the progress_manager namespace.
...
# RUNTIME_CONTEXT_COMPARE_KEYS, _normalize_context_path, _normalize_optional_string,
# collect_git_context, build_runtime_context, build_execution_context,
# _runtime_context_fingerprint, _update_runtime_context, _update_execution_context,
# compare_contexts, _format_context_summary moved to git_context.py (F18 modularisation).
# Re-imported above so existing call sites (and test patches) continue to resolve via
# the progress_manager namespace.
...
# _scope_hint, _extract_command_tail, _discover_parent_route_bindings_for_child,
# _print_route_preflight_block, enforce_route_preflight moved to route_preflight.py
# (F18 modularisation). Re-imported below so existing call sites (and test
# patches) continue to resolve via the progress_manager namespace.
