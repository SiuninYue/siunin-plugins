# Feature 8: /prog-done 收尾门禁 Implementation Plan (v2)

> **Supersedes:** docs/superpowers/plans/2026-03-17-prog-done-gate.md
> **P1 Fixes Applied:**
> 1. CLI 测试调用方式改为 patch sys.argv 调用 main() 或执行脚本路径
> 2. "done" 命令添加到 MUTATING_COMMANDS 集合
> 3. get_state_dir() 调用传递正确的 target_root 参数
> 4. 验收测试目录使用 find_project_root() 而非 os.getcwd()
> 5. 职责边界：skill 委托给新命令，skill 负责编排型流程，cmd_done 负责确定性门禁与状态写入
> 6. 报告目录统一为 docs/progress-tracker/state/test_reports/
> 7. Task 1 测试逻辑修正：stub 实现与断言对齐
...
**Architecture:** Add `done` subcommand to progress_manager.py that validates preconditions, runs acceptance tests from feature's test_steps, saves detailed test reports, and either completes the feature (calling existing `complete_feature`) or records finish_pending status with failure reasons. The `/prog-done` skill will delegate to this command for deterministic gatekeeping while handling orchestration workflows.
...
**Create:**
- `plugins/progress-tracker/docs/progress-tracker/state/test_reports/` - Directory for test reports (created dynamically)
...
- Modify: `plugins/progress-tracker/hooks/scripts/progress_manager.py:126-130` (add "done" to MUTATING_COMMANDS)
...
def test_done_command_no_active_feature(temp_dir):
    ...
    os.chdir(temp_dir)
    try:
        # Import and run done command via main() with patched argv
        import sys
        from unittest.mock import patch

        # Patch argv to simulate command
        with patch.object(sys, 'argv', ['progress_manager', 'done']):
            from plugins.progress_tracker.hooks.scripts.progress_manager import main
            # Capture output
            import io
            ...
            # Assertions
            assert exit_code == 1
            assert "无活动功能" in output or "no active feature" in output
    finally:
...
Expected: FAIL with "NameError: name 'main' is not defined" or similar (command not implemented)

- [ ] **Step 3: Add "done" to MUTATING_COMMANDS**

```python
# Around line 126 in progress_manager.py
MUTATING_COMMANDS = {
    "init", "next", "start", "complete", "undo", "sync", "fix", "done"  # Add "done"
}
```

- [ ] **Step 4: Add done command parser**
...
- [ ] **Step 5: Add done command handler**
...
- [ ] **Step 6: Add stub cmd_done function**
...
def cmd_done(commit_hash=None, run_all=False, skip_archive=False):
    ...
    print("[DONE] 无活动功能，请先运行 /prog next")
    return 1  # Error code for no active feature
```

- [ ] **Step 7: Run test to verify it passes**

Run: `pytest plugins/progress-tracker/tests/test_progress_manager.py::test_done_command_no_active_feature -v`
Expected: PASS (command exists and returns correct error code)
...
git commit -m "feat: add done command stub to MUTATING_COMMANDS"
```
...
def test_done_command_wrong_workflow_phase(temp_dir):
    ...
    os.chdir(temp_dir)
    try:
        # Use script execution approach (consistent with existing tests)
        script_path = Path(__file__).parent.parent.parent / "hooks" / "scripts" / "progress_manager.py"
        result = subprocess.run(
            [sys.executable, str(script_path), "done"],
            capture_output=True,
            text=True,
            cwd=temp_dir  # Explicit cwd instead of os.chdir
        )
...
def test_done_command_runs_acceptance_tests(temp_dir):
    ...
    # Use find_project_root instead of os.getcwd() for project scope
    ...
    script_path = Path(__file__).parent.parent.parent / "hooks" / "scripts" / "progress_manager.py"
    result = subprocess.run(
        [sys.executable, str(script_path), "done", "--run-all"],
        capture_output=True,
        text=True,
        cwd=cwd  # Use project root for proper scope
    )
...
def _run_acceptance_tests(feature_id: int, run_all: bool = False) -> Tuple[bool, List[TestResult]]:
    ...
    for step in test_steps:
        ...
        try:
            # Use find_project_root to get correct working directory
            ...
            result = subprocess.run(
                ...
                cwd=cwd  # Use project root instead of os.getcwd()
            )
...
def test_done_command_saves_test_report(temp_dir):
    ...
    project_root = find_project_root(temp_dir)
    cwd = project_root if project_root else temp_dir
...
def _save_test_report(feature_id: int, results: List[TestResult], success: bool):
    ...
    # Get state directory with correct target_root
    try:
        state_dir = get_state_dir(Path.cwd())  # Pass current directory as target_root
    except:
        # Fallback: use project root
        project_root = find_project_root(Path.cwd())
        if project_root:
            state_dir = project_root / "docs" / "progress-tracker" / "state"
        else:
...
def test_complete_feature_via_done_command(self, feature_in_planning):
    ...
    # Instead of calling skill, use done command via script execution
    script_path = Path(__file__).parent.parent.parent / "hooks" / "scripts" / "progress_manager.py"
    result = subprocess.run(
        [sys.executable, str(script_path), "done"],
        capture_output=True,
...
Search for tests that use "prog_done" or "complete" and update to use actual command via script execution.
...
**Files:**
- Modify: `plugins/progress-tracker/commands/prog-done.md` (update to reflect skill delegation architecture)
...
**Architecture:**
This command delegates to the `cmd_done()` function in `progress_manager.py` for deterministic gatekeeping, while the `feature-complete` skill handles orchestration workflows (human verification dialogue, final prompts, project_memory append, and branch cleanup).
...
git commit -m "docs: update prog-done command documentation with skill delegation"
```
...
**Key changes from v1:**
- All P1 issues fixed (CLI test calls, MUTATING_COMMANDS, get_state_dir API, project scope, skill delegation)
- P2 inconsistencies resolved (report directory, test logic)
- Clear architecture: skill delegates to cmd_done for deterministic gatekeeping
